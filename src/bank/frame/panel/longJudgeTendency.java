...
public class longJudgeTendency extends BaseFrame {
	...
	public longJudgeTendency(String bankname, String year) {
		...
		rangeDeposit = new JMenuItem("存款稳定性排名");
		gradeDeposit = new JMenuItem("存款稳定性等级");
		resultDeposit = new JMenuItem("存款稳定性");
		rangeLoan = new JMenuItem("贷款稳定性排名");
		gradeLoan = new JMenuItem("贷款稳定性等级");
		resultLoan = new JMenuItem("贷款稳定性");

		work = new MyPanel[6];
		// System.out.println("test/"+bankname+"长期存款归一化值"+".jpg");
		work[0] = new MyPanel("test/" + bankname + "长期存款稳定性" + ".jpg");
		work[1] = new MyPanel("test/" + bankname + "长期贷款稳定性" + ".jpg");
