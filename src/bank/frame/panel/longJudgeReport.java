...
public class longJudgeReport extends workPanel {
	...
	public longJudgeReport(JudgeRecord record) {
		...
		depositl = new JLabel("    存款量:");
		deposit = new JLabel(record.getDeposit());
		loanl = new JLabel("    贷款量:");
	...
	public void generateTendencyImage() throws Exception {
		...
		resultDeposit.setImageName(banknameString + "长期存款" + "稳定性");
		try {
		...
		resultLoan.setImageName(banknameString + "长期贷款" + "稳定性");
		try {
	...
	public boolean checkImageExist() {
		...
		for (int i = 0; i < fileList.length; i++) {
			...
			if (fileList[i].equals(record.getBankname() + "长期存款稳定性" + ".jpg")
					|| fileList[i].equals(record.getBankname() + "长期贷款稳定性"
							+ ".jpg")
					|| fileList[i].equals(record.getBankname() + "长期存款稳定性排名"
							+ ".jpg")
					|| fileList[i].equals(record.getBankname() + "长期贷款稳定性排名"
							+ ".jpg")
					|| fileList[i].equals(record.getBankname() + "长期存款稳定性等级"
							+ ".jpg")
					|| fileList[i].equals(record.getBankname() + "长期贷款稳定性等级"
							+ ".jpg")) {
